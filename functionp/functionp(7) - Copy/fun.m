...
dp = zeros(11,1);    % a column vector
    dp(1,1) = 4*sin(k*p(2))*Op*(p(5)-conj(p(5)))+(2*D)^(1/2)*sigma*rand+mu;
    dp(2,1) = p(1)/m;
    dp(3,1) = +G3*p(11)-2*1i*(p(5)-p(9))*Op*cos(k*p(2));
    dp(4,1) = 1i*(Dp)*p(4)-1i*p(5)*Oc+2*1i*p(10)*Op*cos(k*p(2)); 
    dp(5,1) = 1i*Dp*p(5)-((G3)/2)*p(5)-1i*p(4)*Oc+2*1i*(p(11)-p(3))*Op*cos(k*p(2));
    dp(6,1) = conj(dp(4));
    dp(7,1) = -1i*Oc*(p(8)-p(10));
    dp(8,1) = -((G3)/2)*p(8)+1i*(p(11)-p(7))*Oc-2*1i*p(6)*cos(k*p(2));
    dp(9,1) = conj(dp(5));
    dp(10,1) = conj(dp(8));
